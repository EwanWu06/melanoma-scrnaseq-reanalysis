...
**Build-script pattern:** Some notebooks (currently `05_seurat_rpca.ipynb`)
are generated by a Python script (`scripts/build_notebook_NN.py`) rather
than hand-edited. The build script is the source of truth — do not
manually edit the `.ipynb`, as changes will be overwritten on next
regeneration. To modify the notebook, edit the build script and re-run
`python scripts/build_notebook_NN.py`.
